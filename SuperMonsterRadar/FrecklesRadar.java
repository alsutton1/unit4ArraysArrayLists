...
public class FrecklesRadar
{
    ...
    private int[][] prevScan;

    // location of the monster
    private int frecklesLocationRow;
    private int frecklesLocationCol;
    private int Row;
    private int Col;
    private int dx;
    private int dy;
     ...
     */
    public FrecklesRadar(int rows, int cols)
    {
        // initialize instance variables
        currentScan = new boolean[rows][cols]; // elements will be set to false
        accumulator = new int[rows][cols]; // elements will be set to 0
        prevScan = new int [rows][cols];

        // randomly set the location of the monster (can be explicity set through the
        //  setMonsterLocation method
        Row = frecklesLocationRow;
        Col = frecklesLocationCol;
    ...
    public void scan()
    {
        ...
        for(int row = 0; row < currentScan.length; row++)
        {
            for(int col = 0; col < currentScan[0].length; col++)
            {
                for (int i = 0; i < prevScan.length; i++)
                {
                    if(currentScan[row][col] == true)
                    {
     ...
     */
    public void setFrecklesLocation(int row, int col)
    {
        // remember the row and col of the monster's location
        frecklesLocationRow = row;
        frecklesLocationCol = col;
        Row = row;
        Col = col;
    ...
    public void updateFrecklesLocation(int x, int y)
    {
        Row -= x;
        Col += y;
    }
